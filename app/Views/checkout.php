            ...
            <table class="table">
                ...
                <tbody>
                    ...
                    <tr>
                        <td colspan="2"></td>
                        <td>Ongkir</td>
                        <td><span id="ongkir_total"><?php echo number_to_currency(0, 'IDR') ?></span></td>
                    </tr>
                    <tr>
                        <td colspan="2"></td>
                        <td>Biaya Admin</td>
                        <td><span id="biaya_admin"><?php echo number_to_currency(0, 'IDR') ?></span></td>
                    </tr>
                    <tr>
                        <td colspan="2"></td>
                        <td>PPN</td>
                        <td><span id="ppn"><?php echo number_to_currency(0, 'IDR') ?></span></td>
                    </tr>
                    <tr>
                        <td colspan="2"></td>
                        <td>Total</td>
...
<script>
    $('document').ready(function(){
        ...
        $("#service").on('change', function(){
            ...
            // Calculate administrative fee and PPN based on total transaction
            if (total <= 50000) {
                biayaAdmin = total * 0.006;
            } else if (total <= 100000) {
                biayaAdmin = total * 0.008;
            } else {
                biayaAdmin = total * 0.01;
            }
            ...
            $("#ongkir").val(ongkir);
            $("#ongkir_total").html("IDR " + ongkir.toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,'));
            $("#biaya_admin").html("IDR " + biayaAdmin.toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,'));
            $("#ppn").html("IDR " + ppn.toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,'));
            $("#total").html("IDR " + total.toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,'));

            // Update the hidden input value for total_harga
            var totalHarga = total + biayaAdmin + ppn;
